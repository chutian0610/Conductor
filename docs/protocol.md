...
| `{"type":"assistant","message":{...}}`        | assistant turn | iterate `message.content[]`, mapping each block: `text`→`MessageText`, `tool_use`→`MessageToolUse` (with `Input`) |
...
| `{"type":"item.completed","item":{...}}`      | item completes  | `MessageToolResult` for command_execution (aggregated_output + exit_code); `MessageText` for `agent_message` |
