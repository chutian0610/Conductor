...
// TestCodexIntegration_ReasoningEmitsMessageThinking verifies that a
// wire `item` with `type:"reasoning"` surfaces as
// MessageType=MessageThinking on the session channel.
//
// Codex emits reasoning chunks before (and sometimes alongside) the
// final `agent_message`. Conductor converts each `reasoning` item to
// a separate Message{Type: MessageThinking} so the renderer's
// `... <truncated>` line and the registry's per-event storage both
// pick the reasoning up.
func TestCodexIntegration_ReasoningEmitsMessageThinking(t *testing.T) {
	binary := MustBuildFakeBinary(t, "fake-codex")
	script := WriteScript(t, t.TempDir(), "reasoning.jsonl", []ScriptStep{
		{Event: codexThreadStarted},
		{Event: codexTurnStarted},
		// Reasoning chunk first — model thinks out loud.
		{DelayMs: 5, Event: json.RawMessage(
			`{"type":"item.completed","item":{"type":"reasoning",` +
				`"text":"hmm, let me consider the request"}}`,
		)},
		// Then the actual answer.
		{DelayMs: 5, Event: codexAgentMessage},
		// Reasoning chunk again, after the first answer — multi-step.
		{DelayMs: 5, Event: json.RawMessage(
			`{"type":"item.completed","item":{"type":"reasoning",` +
				`"text":"and one more thing..."}}`,
		)},
		{DelayMs: 5, Event: codexTurnCompleted},
	})
	...
	if len(thinking) != 2 {
		t.Fatalf("reasoning events: got %d want 2; got: %v", len(thinking), thinking)
	}
	if len(text) != 1 {
		t.Fatalf("text events: got %d want 1", len(text))
	}
	if thinking[0].Content != "hmm, let me consider the request" {
		t.Errorf("first reasoning content wrong: %q", thinking[0].Content)
	}
	if thinking[1].Content != "and one more thing..." {
		t.Errorf("second reasoning content wrong: %q", thinking[1].Content)
	}
	if text[0].Content != "hello from fake codex" {
